...
export class ApprovalService {
  ...
  async submitTopicForReview(topicId: string, userId: string): Promise<Topics> {
    ...
    if (!isOwner && !isCollaborator) {
      throw new BadRequestException(
        'Only the creator or collaborators can submit this topic for review',
      );
    }
    ...
    if (!editableStates.includes(topic.status)) {
      throw new BadRequestException(
        `Topic cannot be submitted. Current status: ${topic.status}`,
      );
    }
  ...
  async approveTopic(topicId: string, adminId: string): Promise<Topics> {
    ...
    if (topic.status !== 'pending_approval') {
      throw new BadRequestException(
        `Topic is not pending approval. Current status: ${topic.status}`,
      );
    }
  ...
  async requestEditPermission(
    ...
    if (!isOwner && !isCollaborator) {
      throw new BadRequestException(
        'Only the creator or collaborators can request edit permission',
      );
    }

    if (topic.status !== 'approved') {
      throw new BadRequestException(
        `Can only request edit permission for approved topics. Current status: ${topic.status}`,
      );
    }

    if (topic.edit_request_pending) {
      throw new BadRequestException(
        'There is already a pending edit request for this topic',
      );
    }
